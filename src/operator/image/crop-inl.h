...
inline void CropImpl(int x,
  ...
  MXNET_NDIM_SWITCH(data.ndim(), ndim, {
    CHECK(x + width <= data.shape_[ndim - 2]) << "  x + width should not be greater than input width";
    CHECK(y + height <= data.shape_[ndim - 3]) << "y + height should not be greater than input height";
